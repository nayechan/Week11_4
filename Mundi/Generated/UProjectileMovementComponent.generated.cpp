// Auto-generated file - DO NOT EDIT!
// Generated by Tools/CodeGenerator/generate.py

#include "pch.h"
#include "Source/Runtime/Engine/Components/ProjectileMovementComponent.h"
#include "Source/Runtime/Core/Object/ObjectMacros.h"
#include "Source/Runtime/Engine/Scripting/LuaBindHelpers.h"

// ===== Class Factory Registration (IMPLEMENT_CLASS) =====

// IMPLEMENT_CLASS(UProjectileMovementComponent) expansion
namespace {
    struct UProjectileMovementComponentFactoryRegister
    {
        UProjectileMovementComponentFactoryRegister()
        {
            ObjectFactory::RegisterClassType(
                UProjectileMovementComponent::StaticClass(),
                []() -> UObject* { return new UProjectileMovementComponent(); }
            );
        }
    };
    static UProjectileMovementComponentFactoryRegister GRegister_UProjectileMovementComponent;
    static bool bIsRegistered_UProjectileMovementComponent = [](){ UProjectileMovementComponent::StaticClass(); return true; }();
}

// Static member initialization for reflection registration
const bool UProjectileMovementComponent::bPropertiesRegistered = []() {
    UProjectileMovementComponent::StaticRegisterProperties();
    return true;
}();


// ===== Property Reflection =====

BEGIN_PROPERTIES(UProjectileMovementComponent)
    MARK_AS_COMPONENT("발사체 이동 컴포넌트", "발사체 물리 이동 컴포넌트입니다")
    ADD_PROPERTY(float, Gravity, "발사체", true, "발사체 중력 가속도입니다")
    ADD_PROPERTY_METADATA(Gravity, "LuaReadWrite", "true")
    ADD_PROPERTY(FVector, Velocity, "속도", true, "초기 속도입니다")
    ADD_PROPERTY_METADATA(Velocity, "LuaReadWrite", "true")
    ADD_PROPERTY(float, InitialSpeed, "발사체", true, "발사체 초기 속도입니다")
    ADD_PROPERTY_METADATA(InitialSpeed, "LuaReadWrite", "true")
    ADD_PROPERTY(float, MaxSpeed, "발사체", true, "발사체 최대 속도입니다")
    ADD_PROPERTY_METADATA(MaxSpeed, "LuaReadWrite", "true")
    ADD_PROPERTY(float, HomingAccelerationMagnitude, "호밍", true, "호밍 가속도 크기입니다")
    ADD_PROPERTY_METADATA(HomingAccelerationMagnitude, "LuaReadWrite", "true")
    ADD_PROPERTY(bool, bIsHomingProjectile, "호밍", true, "호밍 기능을 활성화합니다")
    ADD_PROPERTY_METADATA(bIsHomingProjectile, "LuaReadWrite", "true")
    ADD_PROPERTY(float, ProjectileLifespan, "발사체", true, "발사체 생명 시간입니다")
    ADD_PROPERTY_METADATA(ProjectileLifespan, "LuaReadWrite", "true")
    ADD_PROPERTY(bool, bAutoDestroyWhenLifespanExceeded, "발사체", true, "생명 시간 초과시 발사체를 파괴합니다")
    ADD_PROPERTY_METADATA(bAutoDestroyWhenLifespanExceeded, "LuaReadWrite", "true")
END_PROPERTIES()

// ===== Lua Binding =====

extern "C" void LuaBind_Anchor_UProjectileMovementComponent() {}

LUA_BIND_BEGIN(UProjectileMovementComponent)
{
    // No functions to bind
}
LUA_BIND_END()

