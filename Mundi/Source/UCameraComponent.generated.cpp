// Auto-generated file - DO NOT EDIT!
// Generated by Tools/CodeGenerator/generate.py

#include "pch.h"
#include "CameraComponent.h"
#include "ObjectMacros.h"
#include "LuaBindHelpers.h"

// ===== Class Factory Registration (IMPLEMENT_CLASS) =====

// IMPLEMENT_CLASS(UCameraComponent) expansion
namespace {
    struct UCameraComponentFactoryRegister
    {
        UCameraComponentFactoryRegister()
        {
            ObjectFactory::RegisterClassType(
                UCameraComponent::StaticClass(),
                []() -> UObject* { return new UCameraComponent(); }
            );
        }
    };
    static UCameraComponentFactoryRegister GRegister_UCameraComponent;
    static bool bIsRegistered_UCameraComponent = [](){ UCameraComponent::StaticClass(); return true; }();
}

// Static member initialization for reflection registration
const bool UCameraComponent::bPropertiesRegistered = []() {
    UCameraComponent::StaticRegisterProperties();
    return true;
}();


// ===== Property Reflection =====

BEGIN_PROPERTIES(UCameraComponent)
    MARK_AS_COMPONENT("카메라 컴포넌트", "카메라 뷰포트 컴포넌트입니다")
    ADD_PROPERTY_RANGE(float, FieldOfView, "Camera", 1.0f, 179.0f, true)
    ADD_PROPERTY_METADATA(FieldOfView, "LuaReadWrite", "true")
    ADD_PROPERTY_RANGE(float, AspectRatio, "Camera", 0.1f, 10.0f, true)
    ADD_PROPERTY_METADATA(AspectRatio, "LuaReadWrite", "true")
    ADD_PROPERTY_RANGE(float, NearClip, "Camera", 0.01f, 1000.0f, true)
    ADD_PROPERTY_METADATA(NearClip, "LuaReadWrite", "true")
    ADD_PROPERTY_RANGE(float, FarClip, "Camera", 1.0f, 100000.0f, true)
    ADD_PROPERTY_METADATA(FarClip, "LuaReadWrite", "true")
    ADD_PROPERTY_RANGE(float, ZoomFactor, "Camera", 0.1f, 10.0f, true)
    ADD_PROPERTY_METADATA(ZoomFactor, "LuaReadWrite", "true")
END_PROPERTIES()

// ===== Lua Binding =====

extern "C" void LuaBind_Anchor_UCameraComponent() {}

LUA_BIND_BEGIN(UCameraComponent)
{
    // No functions to bind
}
LUA_BIND_END()

