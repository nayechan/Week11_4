...
protected:
	UPROPERTY(LuaReadWrite, EditAnywhere, Category="Light", Range="1000.0, 15000.0", Tooltip="조명의 색온도를 켈빈(K) 단위로 설정합니다\n(1000K: 주황색, 6500K: 주광색, 15000K: 푸른색)")
	float Temperature = 6500.0f; // 색온도 (K)

	UPROPERTY(LuaReadWrite, EditAnywhere, Category="Light", Range="512, 8192", Tooltip="Shadow Resolution Scale")
	int ShadowResolutionScale = 1024;

	UPROPERTY(LuaReadWrite, EditAnywhere, Category="Light", Range="0.0, 0.01", Tooltip="Shadow Bias")
	float ShadowBias = 0.00001f;

	UPROPERTY(LuaReadWrite, EditAnywhere, Category="Light", Range="0.0, 0.01", Tooltip="Shadow Slope Bias")
	float ShadowSlopeBias = 0.00001f;

	UPROPERTY(LuaReadWrite, EditAnywhere, Category="Light", Range="0.0, 1.0", Tooltip="Shadow Sharpen - 0.0f(Soft) ~ 1.0f(Sharp)")
