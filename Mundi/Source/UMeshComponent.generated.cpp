// Auto-generated file - DO NOT EDIT!
// Generated by Tools/CodeGenerator/generate.py

#include "pch.h"
#include "Source/Runtime/Engine/Components/MeshComponent.h"
#include "Source/Runtime/Core/Object/ObjectMacros.h"
#include "Source/Runtime/Engine/Scripting/LuaBindHelpers.h"

// ===== Class Factory Registration (IMPLEMENT_CLASS) =====

// IMPLEMENT_CLASS(UMeshComponent) expansion
namespace {
    struct UMeshComponentFactoryRegister
    {
        UMeshComponentFactoryRegister()
        {
            ObjectFactory::RegisterClassType(
                UMeshComponent::StaticClass(),
                nullptr
            );
        }
    };
    static UMeshComponentFactoryRegister GRegister_UMeshComponent;
    static bool bIsRegistered_UMeshComponent = [](){ UMeshComponent::StaticClass(); return true; }();
}

// Static member initialization for reflection registration
const bool UMeshComponent::bPropertiesRegistered = []() {
    UMeshComponent::StaticRegisterProperties();
    return true;
}();


// ===== Property Reflection =====

BEGIN_PROPERTIES(UMeshComponent)
    ADD_PROPERTY_ARRAY(EPropertyType::Material, MaterialSlots, "Materials", true, "Material slots for the mesh")
    ADD_PROPERTY_METADATA(MaterialSlots, "LuaReadWrite", "true")
    ADD_PROPERTY_METADATA(MaterialSlots, "inner_type", "EPropertyType::Material")
    ADD_PROPERTY(bool, bCastShadows, "Rendering", true, "그림자를 드리울지 여부입니다")
    ADD_PROPERTY_METADATA(bCastShadows, "LuaReadWrite", "true")
END_PROPERTIES()

// ===== Lua Binding =====

extern "C" void LuaBind_Anchor_UMeshComponent() {}

LUA_BIND_BEGIN(UMeshComponent)
{
    AddAlias<UMeshComponent, const uint32, const FString&, const FLinearColor&>(
        T, "SetColor", &UMeshComponent::SetMaterialColorByUser);
    AddAlias<UMeshComponent, const uint32, const FString&, float>(
        T, "SetScalar", &UMeshComponent::SetMaterialScalarByUser);
}
LUA_BIND_END()
