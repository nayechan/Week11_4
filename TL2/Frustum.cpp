...
namespace
{
    ...
    Plane MakePlane(const FVector& Point, const FVector& Normal)
    {
        const FVector UnitNormal = Normal.GetSafeNormal(); // 길이 1로 보정
        return Plane
        {
            UnitNormal,
            FVector::Dot(UnitNormal, Point)
        };
    }
...
Frustum CreateFrustumFromCamera(const UCameraComponent& Camera, float OverrideAspect)
{
    ...
    const FVector Origin = Camera.GetWorldLocation();
    ...
    const FVector FrontMultFar = Forward * FarClip;        // Far까지의 전방 벡터
    ...
    // ------------------------------------------------------------
    Result.RightFace = MakePlane(Origin, FVector::Cross(FrontMultFar + Right * HalfHSide, Up));
    Result.LeftFace = MakePlane(Origin, FVector::Cross(Up, FrontMultFar - Right * HalfHSide));
    Result.TopFace = MakePlane(Origin, FVector::Cross(Right, FrontMultFar + Up * HalfVSide));
    Result.BottomFace = MakePlane(Origin, FVector::Cross(FrontMultFar - Up * HalfVSide, Right));
    return Result;
}
...
bool Intersects(const Plane& P, const FVector& Center, const FVector& Extents) 
{
    ...
    const float Distance = FVector::Dot(P.Normal, Center) - P.Distance;
...
{
    // AABB 중심/반길이
    const FVector Center = (Bound.Min + Bound.Max) * 0.5f;
    const FVector Extents = (Bound.Max - Bound.Min) * 0.5f; // 항상 양수

    // 6면 모두 통과해야 절두체의 안쪽이므로 "보이는 것"
    return Intersects(Frustum.LeftFace, Center, Extents) && Intersects(Frustum.RightFace, Center, Extents)
        && Intersects(Frustum.TopFace, Center, Extents) && Intersects(Frustum.BottomFace, Center, Extents)
        && Intersects(Frustum.NearFace, Center, Extents) && Intersects(Frustum.FarFace, Center, Extents);
}
